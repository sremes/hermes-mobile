...
// Fork note: upstream imports this from '../electron/pool-limits' — the
// Electron shell is stripped here, so the shape is inlined. Keep in sync
// with electron/pool-limits.ts (maxBackends: max spawned non-primary profile
// backends; idleMs: idle lifetime of an unused pool backend).
interface PoolLimits {
  idleMs: number
  maxBackends: number
}

// Fork note: upstream keeps these in '../electron/command-screenshot-types' —
// the Electron shell is stripped here, so the shapes are snapshotted. Keep in
// sync with electron/command-screenshot-types.ts. The PWA never provides the
// `screenshot` bridge member (macOS native screen capture has no web
// equivalent); callers feature-detect `window.hermesDesktop?.screenshot`.
...
// Fork note: upstream keeps this in '../electron/notification-types' — the
// Electron shell is stripped here, so the shape is snapshotted. Keep in sync
// with electron/notification-types.ts.
